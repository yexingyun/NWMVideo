...
class Scraper:
    ...
    async def convert_share_urls(self, url: str) -> Union[str, None]:
        ...
        if 'douyin' in url:
            ...
            if 'v.douyin' in url:
                ...
                try:
                    response = http_utils.get(url, param=None, header=self.headers, redirect=False)
    ...
    async def get_tiktok_video_data(self, video_id: str) -> Union[dict, None]:
        ...
        try:
            ...
            #                            timeout=10) as response:
            response = http_utils.get(api_url, param=None, header=self.tiktok_api_headers, redirect=False)
            response_json = response.json()
            video_data = response_json['aweme_list'][0]
            # print('获取视频信息成功！')
            return video_data
        except Exception as e:
